...
import { startUserUpdate } from "../../reduxStore/actions/usersAction"
    ...
    const handleFormSubmit = (e) => {
        ...
        if (Object.keys(formErr).length > 0) {
            ...
            const resolve = () => {
                setUserData("")
                setFormErrors({})
                window.alert(`"Successfully Updateed`)
    ...
    return (
        <div>
            <Form onSubmit={handleFormSubmit}>
                ...
                {
                    userNameToggle && <button onClick={handleUserNameToggle} className="btn btn-secondary mt-3 ms-3">Cancle</button>
                }
                 {
                    mobileToggle && <button onClick={handleMobileToggle} className="btn btn-secondary mt-3 ms-3">Cancle</button>
                }
            </Form>
        </div>
    )
